...
function buildBoxShadow(glowColor: string, intensity: number): string {
    const isVar = glowColor.includes('var(');
    let base = '';
    if (!isVar) {
    ...
    return layers.map(([x, y, blur, spread, alpha, inset]) => {
        ...
        if (isVar) {
            return `${inset ? 'inset ' : ''}${x}px ${y}px ${blur}px ${spread}px rgba(${glowColor}, ${a / 100})`;
        }
...
const BorderGlow: React.FC<BorderGlowProps> = ({
    ...
    return (
        ...
        >
            ...
            {/* mesh gradient fill near edges - DISABLED AS USER WANTS ONLY OUTER GLOW */}
            {/* 
      <div
        className="absolute inset-0 rounded-[inherit] -z-[1]"
        style={{
